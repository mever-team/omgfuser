...
from omgfuser.models.dctconfig.default import update_config
from omgfuser.models.dctconfig import config
...
class DCTStream(nn.Module):
    ...
    def __init__(self, include_final_layers: bool = False):
        self.include_final_layers: bool = include_final_layers

        current_dir: pathlib.Path = pathlib.Path(__file__).parent
        args = argparse.Namespace(
            cfg=str(current_dir/"dctconfig/CAT_DCT_only.yaml"),
            opts=['TEST.FLIP_TEST', 'False',
                  'TEST.NUM_SAMPLES', '0']
        )
    ...
    def forward(self, x, qtable):
        ...
        B, C, H, W = x.shape
        x0 = x.reshape(B, C, H // 8, 8, W // 8, 8).permute(0, 1, 3, 5, 2, 4).reshape(
            B, 64 * C, H // 8, W // 8
        )  # [B, 256, 32, 32]
        ...
        if self.include_final_layers:
            x = self.last_layer(x)

        return x

    def init_weights(self, pretrained_dct='',):
        ...
        if os.path.isfile(pretrained_dct):
            loaded_dict = torch.load(pretrained_dct, map_location="cpu")['state_dict']
            model_dict = self.state_dict()
            loaded_dict = {k: v for k, v in loaded_dict.items()
                               if k in model_dict.keys()}
            loaded_dict = {k:v for k,v in loaded_dict.items()
                           if not k.startswith('last_layer')}
            logger.info('=> (DCT) loading pretrained model {} ({})'.format(
